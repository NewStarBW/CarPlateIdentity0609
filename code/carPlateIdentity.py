...
if __name__ == '__main__':
    ...
    for image_file in image_files:
        ...
        print(f"图像 {image_file} 中的文本：{text}")

        # 可以选择保存处理后的图像或结果到文件
        # cv2.imwrite(os.path.join(output_folder, image_file), car_plate)
        # with open('result.txt', 'a') as f:
        #     f.write(f"{image_file}: {text}\n")
